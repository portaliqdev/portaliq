...
/**
 * Staff sign-up. For a private tool you'll likely create accounts here once
 * then restrict further sign-ups (Better Auth config / a allowlist) before launch.
 */
    ...
    <div className="flex min-h-screen items-center justify-center bg-canvas px-4">
      <form onSubmit={onSubmit} className="w-full max-w-sm space-y-4 rounded-lg border border-hairline bg-surface-1 p-6">
        <div>
          ...
          <p className="mt-1 text-[13px] text-ink-muted">Create a staff account.</p>
        </div>
        ...
        <p className="text-center text-[12px] text-ink-muted">
          Have an account? <Link href="/auth/sign-in" className="text-md-red hover:underline">Sign in</Link>
        </p>
      </form>
    </div>
  );
